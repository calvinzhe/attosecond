%SFA_H_3D_VORTEX_tau_sweep.m: SFA, real hydrogen atom, Feb20 2017
%works
clear,clc,tic
close all
I1=sqrt(-1);
...
for i_tau=1:105
    ...
    str = {strcat('$$\tau = ', num2str(round(tau,1)), '\ au$$'), ...
            strcat('$$T = ', num2str(round(T,1)), '\ au$$'), ...
            strcat('$$\lambda = ', num2str(round(lambda,1)),'\ nm$$')};
    text(-0.95,0.75,str,'Interpreter','latex','BackgroundColor','yellow');
    pbaspect([1 1 1])


    saveas(fig,strcat('./CCP_RL_tau_only_scan/p_vs_tau_-_tau_',num2str(round(tau,2)*100),'.png'));
