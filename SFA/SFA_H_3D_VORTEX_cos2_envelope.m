...
nt=500;
W= 1.3245; %3/27.2;    %0.1103 angular frequency => Period = 2*pi/W = 56.9675 => F = 0.0176
lambda = (3*10^8)/(W/(2*pi))*(2.419*10^-17)/10^-9;  %wavelength in nm
nc = 6;
T=nc*2*pi/W;  %Pulse width = N*Period = 6*[2*3.14/(3/27.2)] = 341.8 | e^(-T^2/T^2) = e^(-1) = 0.3679
%T=2000; %pulse width in atomic units
tau=1.163*T;       %Pulse separation
phi1 = 0;
phi2 = pi/2;
E0=sqrt(3.5e15/3.5e16);
Ip=0.9037;  %H=12.13/27.2  |  He=0.9037 for He
ksi=1;
...
pc = 1; p1 = 1; p2 = 1;
if strcmp(polarization, 'RL')
    p1 = -1; p2 = 1;
elseif strcmp(polarization, 'LR')
    p1 = 1; p2 = -1;
